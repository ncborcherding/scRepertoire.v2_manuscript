...
plot_qc_metrics <- function(seurat_obj, file_name, qc_dir) {
  ...
  save_plot(p, file.path(qc_dir, paste0(file_name, "_metrics.pdf")), height = 8, width = 8)
}
...
filter_cells <- function(seurat_obj, file_name, qc_dir) {
  ...
  save_plot(p, file.path(qc_dir, paste0(file_name, "_cutpoint.pdf")), height = 8, width = 8)
...
object.merge$Cluster_Types <- factor(object.merge$Cluster_Types, sort(unique(object.merge$Cluster_Types))[c(2:12,1,13)])
...
df <- object.merge[[]] %>%
  mutate(Lineage = case_when(
    Cluster_Types %in% c("Plasma_Cell", "B Cells") ~ "B Cell",
    Cluster_Types %in% c("CD4_TCM/Naive", "CD4_T1_Inf_Stimulated", "CD4_TH17",
                         "CD8_Exhausted", "CD4_Treg", "CD8_TEM", "CD8_CTL", 
                         "CD8_TCM/Naive", "CD4/CD8_Proliferating", "MAIT", 
                         "CD4_TEM") ~ "T Cell",
    TRUE ~ "Other" # For any unexpected types, categorize as Other
  ))

# Count the number of each Lineage by orig.ident
lineage_counts <- df %>%
  group_by(orig.ident, Lineage) %>%
  summarise(Count = n(), .groups = "drop")

plot3 <- ggplot(lineage_counts, aes(x = Lineage, y = Count)) +
              geom_boxplot(alpha = 0.4) +
              geom_jitter(aes(color = orig.ident)) + 
              scale_color_viridis(option = "inferno", discrete = TRUE) + 
              labs(x = "Original Identifier (orig.ident)",
                   y = "Cell Count",
                   color = "Sample") +
              theme_minimal() + 
              custom_theme 
...
plot3
ggsave("output/Figure2B_P2.pdf", height = 3, width = 2.5, dpi = 300)
...
ggsave("output/Figure2C.pdf", height = 2, width = 5)
```
...
ggsave("output/Figure2D.pdf", height = 2, width = 4.25)
```
...
plot1 + plot2 + plot_layout(ncol = 2, 
                                            widths = c(1.3, 1))
ggsave("output/Figure2E.pdf", height = 4, width = 9)
```
...
plot1 + plot2 + plot_layout(ncol = 2, guides = "collect", widths = c(3, 1))
ggsave("output/Figure2F.pdf", height = 4, width = 6)
```
...
DimPlot(object.TCells, group.by = "TRA_cluster", pt.size = 0.2, split.by = "Patient", order = TRUE) +
